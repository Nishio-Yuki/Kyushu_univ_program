...
def plot_umap(embs_df, emb_dims, label, output_file, kwargs_dict):
    ...
    # ← ここを変更：save を使わない
    sc.pl.umap(adata, color=label, show=False, **default_kwargs_dict)

    # 親ディレクトリが無ければ作る
    Path(output_file).parent.mkdir(parents=True, exist_ok=True)

    # 直指定で保存（サブフォルダ「umap」を作らない）
        ...
        if plot_style == "umap":
            for label in self.labels_to_plot:
                if label not in emb_labels:
                    ...
                    continue
                # 直下に <output_prefix>.pdf を作る（= STMS_ENMC.pdf にしたければ output_prefix="STMS_ENMC" を渡す）
                output_file = (Path(output_directory) / f"{output_prefix}.pdf")
                plot_umap(embs, emb_dims, label, output_file, kwargs_dict)
                break  # ← 複数ラベルがあっても1枚だけ保存したいなら付ける。不要なら削除
